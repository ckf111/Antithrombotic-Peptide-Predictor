...
def main():
    ...
    # Cache to avoid duplicate API calls for the same ID
    ...
    with open(log_file, "w") as log:
        for seq, header in tqdm(unique_data.items(), desc="Processing sequences"):
            uniprot_id = None
            if "|" in header:
                parts = header.split("|")
                if len(parts) >= 2:
            ...
            signal_end = 0
            if uniprot_id:
                if uniprot_id in signal_peptide_cache:
                    signal_end = signal_peptide_cache[uniprot_id] or 0
                else:
                    signal_info = get_signal_peptide_uniprot(uniprot_id)
                    if signal_info:
                        signal_end = signal_info[1]
                    signal_peptide_cache[uniprot_id] = signal_end
                    # Save cache periodically
                    if len(signal_peptide_cache) % 10 == 0:
